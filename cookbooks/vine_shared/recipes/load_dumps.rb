...
if node.chef_environment == 'dev'
  ...
  execute "import mysql dashdash database" do
    command "mysql -h #{node.run_state['config']['mysql']['host']}  -u #{node.run_state['config']['mysql']['root_user']} -p#{node.run_state['config']['mysql']['root_password']} dashdash < #{node['dirs']['other']}/mysql_dump.sql"
    action :run
  end
end
